...
from ..reactions import bonding_legacy
...
def get_port_info(atom : RDAtom) -> PortInfo:
    ...
    return PortInfo(
        port=atom,
        port_id=atom.GetIdx(),
        port_map_num=atom.GetAtomMapNum(),
...
def splice_port(rdmol : RDMol, sat_group : RDMol) -> RDMol:
    ...
    product = bonding_legacy.increase_bond_order(combomol, *bond_atom_ids, prioritize_unlabelled_ports=False) # must be set to False to ensure correct bonding location
