...
`Espaloma-charge <https://github.com/choderalab/espaloma-charge>`_ (outdated, NOT recommended)
------------------------------------------------------------------
This is a standalone graph neural network (GNN) model [`Wang et. al. <https://pubs.acs.org/doi/10.1021/acs.jpca.4c01287>`_]
which can assign atomic partial charges trained on AM1-BCC data extremely rapidly. 
Due to lack of maintenance and dependency conflicts with Python 3.12, using espaloma-charge with `polymerist` is not longer encouraged,
and support has been kept purely for backwards-compatiblity with legacy code.
To install individually, follow the
